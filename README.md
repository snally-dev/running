...
by Strava Activity ID. If two exports contain the same activity, the export with
...
empty `data/raw/strava/` preserves the existing public CSV.

Precise start/end coordinates stay in ignored raw data and are not written to the
public CSV. BigDataCloud location results are cached in the ignored
